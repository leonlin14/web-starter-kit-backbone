...
require([
  ...
  app.showView = Backbone.View.extend({
    el: '#app', //給範圍(在這個範圍裡面的動作)
    events: {
      'click #menu': 'playRound',
      'click .bottom-menu': 'changeContent',
      'click .scrollTop': 'goTop'
    },
    playRound: function() {
      ...
      if(this.$el.find(".navdrawer-container").hasClass("open")) {
        ...
        this.$el.find(".app-bar").removeClass("open");
        this.$el.find(".navdrawer-bg").removeClass("open");
      } else {
        this.$el.find(".navdrawer-container").addClass("open");
        this.$el.find(".app-bar").addClass("open");
        this.$el.find(".navdrawer-bg").addClass("open");
      }  
    },
    changeContent: function(e) {
      this.$el.find(".bottom-menu").removeClass("selected");
